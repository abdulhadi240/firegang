...
import {
  ...
  Search, EyeOff, Eye, CheckCircle2, Pencil,
} from 'lucide-react'
...
type FilterKey = 'all' | 'attention' | 'matched' | 'excluded'
...
export function ReconciliationClient({ practiceName, reconciliation, initialRows }: Props) {
  ...
  const needsAttention = useCallback(
    (r: GhlReconciliationRow) =>
      r.source !== 'matched' || !r.data[OUR_RECORDING_COLUMN]?.trim(),
    []
  )

  const visibleRows = useMemo(() => {
    let list = rows
    if (filter === 'attention')     list = list.filter((r) => !r.excluded && needsAttention(r))
    else if (filter === 'matched')  list = list.filter((r) => !r.excluded && !needsAttention(r))
    ...
    return list
  }, [rows, filter, search, needsAttention])
  ...
  async function verify() {
    ...
    try {
      ...
      setBanner({ kind: 'ok', text: `${payload.submitted_rows} calls sent for auditing.` })
      router.refresh()
    } catch (err: unknown) {
  ...
  const stats = [
    ...
    { label: 'No recording URL',  value: summary.missing_recording, sub: 'blocks the audit', color: 'bg-red-500',   bg: 'bg-red-50',   border: 'border-red-100' },
  ]

  const activeCount  = rows.filter((r) => !r.excluded).length
  const blockedCount = summary.missing_recording
    ...
    <div>
        ...
        {stats.map((stat, i) => (
          ...
          >
            ...
            <p className={cn(
              ...
              stat.label === 'No recording URL' && stat.value > 0 ? 'text-red-500' : 'text-gray-900'
            )}>
              {stat.value}
            </p>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <div className="inline-flex items-center gap-1 bg-white border border-gray-200 rounded-xl p-1 shadow-sm">
          {([
            ['all',       `All (${activeCount})`],
            ['attention', `Needs attention (${summary.ghl_only + summary.sheet_only + blockedCount})`],
            ['matched',   'Ready'],
          ...
          <table className="w-full text-sm min-w-[64rem]">
            ...
            <tbody className="divide-y divide-gray-100">
              {visibleRows.length === 0 ? (
                <tr>
                  <td colSpan={GRID_COLUMNS.length + 3} className="text-center py-14 text-gray-400 text-sm">
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      ...
                      !recording && !row.excluded && 'bg-red-50/30'
                    )}
                  >
          ...
          <>
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900">
                  {blockedCount > 0
                    ? `${blockedCount} ${blockedCount === 1 ? 'call is' : 'calls are'} missing a recording URL`
                    : 'All calls have a recording URL'}
                </p>
                <p className="text-xs text-gray-400 mt-0.5">
                  {blockedCount > 0
                    ? 'Add the URLs, or exclude those rows, then verify. You can still submit as-is.'
                    : `${activeCount} calls ready to audit.`}
                </p>
              </div>
            </div>
            ...
            >
              ...
              {verifying ? 'Sending…' : `Verify & send ${activeCount} calls`}
            </button>
          </>
        )}
      </div>
